...
class RandomEntry:
    ...
    def random_sensor_string():
        ...
        sensor_string = (str(ph_sensor) + "&" + str(temp_sensor) + "&" + str(do_sensor) + "&" + str(baro_sensor) + "NULL")
        return sensor_string

    @staticmethod
    def random_timestamp_string():
        ...
        timestampnum = f"{year:04d}-{month:02d}-{day:02d}T{hour:02d}:{minute:02d}:{second:02d}.{millisecond:03d}"
        timestamp = str(timestampnum)
        return timestamp

    # Creates a media entry with some random variables
    @staticmethod
    def rand_media_entry():
    ...
    def rand_media_metadata():
        ...
        pH_data, temp_data, dissolved_oxygen_data, barometer_data = SensorString.sensor_string(sensor_data)
        return MediaMetadata.mediadata(left_camera_media, right_camera_media, time,  pH_data, temp_data, dissolved_oxygen_data, barometer_data)
